...
y=sign(qucl(x,A0,b0,c0)) #original labels according to quadratic decision rule
...
for(k in (1:10)*5){
  ...
  for(i in 1:t){
    ...
    err=err+mean(sign(qucl(xtest%*%t(B),Q[[1]],Q[[2]],Q[[3]]))==ytest)/t #average 0-1 empirical error
  }
  cat('Only',t-i,'iterations remain.\n')
  write.table(cbind(d=d,n=n,k=k,test=err,time=runtime),
